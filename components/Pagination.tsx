    ...
    <ul className="bg-muted-100 rounded-full px-1 dark:border-muted-600 dark:bg-muted-700 mb-4 inline-flex flex-wrap gap-2  p-1 md:mb-0 md:gap-1">
      ...
      <li>
        <form action="/" method="GET" onSubmit={(e)=>{
          e.preventDefault();
         const page = (e.target['page'] as HTMLInputElement).value
         router.push(buildHref(parseInt(page)) as string)
          ...
          <input
            ...
            className="w-16 mx-2 nui-focus border-muted-300 text-muted-600 placeholder:text-muted-300 dark:border-muted-700 dark:bg-muted-900/75 dark:text-muted-200 dark:placeholder:text-muted-500 dark:focus:border-muted-700 peer border bg-white font-sans transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-75 px-2 h-10 py-2 text-sm leading-5 rounded-lg"
          />
        </form>
      </li>
      <li key={lastItem}>
